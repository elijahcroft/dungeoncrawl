/**
 * Player weapons — the single source of truth shared by client and server,
 * imported the same way as `classes.ts` and `boss.ts`. The server needs this
 * table because damage is computed server-side (see server combat pipeline);
 * the client needs it for hitboxes, cooldowns, and art.
 *
...
/** Held-weapon art variants the client sprite renderer can draw (see client gfx/sprites.ts). */
export type WeaponSprite =
  ...
  // render as a separate aim-tracking overlay instead.
  | "none";
...
/** Weapon lookup that always returns a valid def (falls back to the starter weapon). */
export function weaponDef(id: string | undefined): WeaponDef {
  return WEAPONS[id ?? STARTER_WEAPON_ID] ?? WEAPONS[STARTER_WEAPON_ID];
}
